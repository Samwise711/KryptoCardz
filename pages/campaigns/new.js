...
class CampaignNew extends Component {
  ...
  onSubmit = async event => {
    ...
    try {
      ...
      axios
        .post(
          'https://us-central1-cryptocardz-c5066.cloudfunctions.net/uploadFile',
          formData
        )
        .then(res => {
          console.log(res);
      ...
      const etherAmt = web3.utils.toWei(
        this.state.minimumContribution,
        'ether'
      );
      await factory.methods.createCampaign(etherAmt, this.state.cardName).send({
        from: accounts[0]
      });
  ...
  render() {
    return (
      <Layout>
        <h3 style={{ marginTop: '25px' }}>Create a New Card</h3>
